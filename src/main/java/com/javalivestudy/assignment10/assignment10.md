...
### Main 쓰레드
- main() 메소드를 실행하며 시작된다
- Main 쓰레드가 없다면 멀티 쓰레드가 나올 수 없다.
- 멀티쓰레드인 경우 Main 쓰레드가 종료되어도 프로세스는 종료되지 않는다.


---
### 동기화
1. 동기화란? 
   - 공유 영역을 사용하는 스레드가 실행중일 때 같은 공유 데이터를 사용하는 다른 스레드가 실행되지 못하도록 순서를 정해주는 것
   - synchronized 키워드 사용 -> 한번에 하나의 쓰레드만 해당 자원에 접근
...
### 데드락
1. DeadLock(교착상태) 란?
   - 두개 이상의 작업이 서로 상대방의 작업이 끝나길 무한정 기다리는 상태
   
2. DeadLock 이 일어나기 위한 4가지 조건
   - 상호배제 : 프로세스는 하나의 자원에 배타적인 통제권을 갖는다.
   - 점유대기 : 프로세스는 자원을 할당받은 상태에서 다른 자원을 기다린다.
   - 비선점 : 프로세스 작업이 끝날 때 까지 자원을 뺏을 수 없다.
   - 순환대기 : 각 프로세스는 순환적으로 다음 프로세스가 요구하는 자원을 가지고 있다.

3. DeadLock 예방
   - 4가지 조건 중 하나를 막음으로써 동작한다.
   
4. DeadLock 회피
   - 자원 할당 상태를 검사하여 허용할지 판단, 순환대기가 발생하지 않도록 함
   - 대표 알고리즘 : 은행원 알고리즘, 자원할당 그래프 알고리즘